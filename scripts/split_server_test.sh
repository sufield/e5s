...
# Split workloadapi server_test.go into focused files
# Week 3, Days 3-4

set -e

ORIG="internal/adapters/inbound/workloadapi/server_test.go"
BACKUP="internal/adapters/inbound/workloadapi/server_test.go.bak"

# Backup original
cp "$ORIG" "$BACKUP"

echo "Creating server_lifecycle_test.go..."
cat > internal/adapters/inbound/workloadapi/server_lifecycle_test.go << 'EOF'
...
echo "Creating server_handler_test.go..."
cat > internal/adapters/inbound/workloadapi/server_handler_test.go << 'EOF'
package workloadapi_test
...
echo "Creating server_compliance_test.go..."
cat > internal/adapters/inbound/workloadapi/server_compliance_test.go << 'EOF'
package workloadapi_test
...
echo "Removing original server_test.go..."
rm "$ORIG"

echo "✅ Split complete!"
echo ""
echo "Created files:"
echo "  - server_lifecycle_test.go (start, stop, constructor)"
echo "  - server_handler_test.go (request handling and validation)"
echo "  - server_compliance_test.go (interface compliance and concurrency)"
echo ""
