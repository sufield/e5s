Short answer: the approach is solid (dev-only, seeded, read-only control plane that feeds a pure read path: **Attest → Match → Issue**). But there are a few correctness gaps and inconsistencies to fix so the docs match the code and the contracts don’t conflict.

Here’s a tight checklist with exact issues and suggested fixes:

## ✅ Strengths (keep)

* Dev-only via `//go:build dev`.
* Registration as seeded data (factory-owned), runtime is read-only.
* Clear separation of bootstrap vs. factory responsibilities.
* Ports expose only read paths; seeding remains an internal concern.
* Good invariants around selector AND-semantics and identity issuance.
