...
**Note:** Docker images will be automatically published to GitHub Container Registry (GHCR) when a release is created using GoReleaser. Until the first release is published, you'll need to build images locally for testing. See [examples/minikube-lowlevel/](../examples/minikube-lowlevel/) for local development setup.
